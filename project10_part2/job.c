...
struct job * add_job(struct job *scheduler, char *job_name, char *user_name, int num_cpus, int num_gpus, int memory, double time, int priority) {
  ...
  struct job *temp = scheduler; //add a temporary pointer to keep track
  struct job *prev = NULL; //prev pointer to track the job with the same priority
  int key = new_job -> priority;

  if(scheduler == NULL || key > scheduler -> priority){ //if job needs to be added at the first place
    new_job -> next = scheduler;
    scheduler = new_job;
    return scheduler;
  }
  while(temp ->next!=NULL && temp ->next -> priority >= key){ //loop around the list to find the right place to put the job
    if(temp -> next -> priority == key){ // incase priority is the same
      prev = temp -> next;
    }
    temp = temp -> next;
    
  }
  if (prev != NULL) { //for same priority
      new_job->next = prev->next;
      prev->next = new_job;
    }
  else{ //to add the job in a sorted list
